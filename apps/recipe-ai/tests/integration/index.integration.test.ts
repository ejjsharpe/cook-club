...
function htmlResponse() {
  return new Response(sampleRecipeHtml, {
    headers: { "content-type": "text/html; charset=utf-8" },
  });
}

describe("RecipeAI Integration Tests", () => {
  let parser: RecipeAI;
  ...
  beforeEach(() => {
    ...
    parser = new RecipeAI(createExecutionContext(), mockEnv);
  });
  ...
  describe("URL parsing with AI", () => {
    beforeEach(() => {
      vi.stubGlobal(
        "fetch",
        vi.fn().mockResolvedValue(htmlResponse()),
      );
    });
  ...
  describe("KV caching", () => {
    ...
    it("caches parsed recipes in KV", async () => {
      ...
      const parser2 = new RecipeAI(createExecutionContext(), mockEnv);
