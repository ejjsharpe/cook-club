...
/**
 * Replace an owned recipe's editable fields and detail rows.
 */
  ...
  return await db.transaction(async (tx) => {
    ...
    await tx
      ...
      .set({
        ...
        description: input.description || null,
        sourceUrl: input.sourceUrl || null,
        sourceType: input.sourceType || existingRecipe.sourceType || "manual",
        prepTime: input.prepTime || null,
        cookTime: input.cookTime || null,
        totalTime: input.totalTime || null,
        servings: input.servings,
    ...
    await tx.delete(recipeImages).where(eq(recipeImages.recipeId, input.id));
    await tx
      .delete(ingredientSections)
      .where(eq(ingredientSections.recipeId, input.id));
    await tx
      .delete(instructionSections)
      .where(eq(instructionSections.recipeId, input.id));
    ...
    for (
      ...
      if (!insertedSection) {
        throw new ServiceError(
          "INTERNAL_ERROR",
          "Failed to update ingredient section",
        );
      }
    ...
    for (
      ...
      if (!insertedSection) {
        throw new ServiceError(
          "INTERNAL_ERROR",
          "Failed to update instruction section",
        );
      }
